...
class LongShortTrader():
    ...
    def __init__(self, symbol, bar_length, return_thresh, volume_thresh, units, position=0):
        ...
        self.trades = 0
        self.trade_values = []
        stop_trading_flag = False  # Flag to control trading thread
    ...
    def start_trading(self, historical_days):
        ...
        while stop_trading_flag is False:
            self.twm.start()
            if self.bar_length in self.available_intervals:
                self.get_most_recent(symbol=self.symbol, interval=self.bar_length,
                                                             days=historical_days)
                self.twm.start_kline_socket(callback=self.stream_candles, symbol=self.symbol,   
    ...
    def report_trade(self, order, going):
        ...
        trade_report_data = {
            ...
            'btc_balance' : balances.loc[balances['asset'] == 'BTC', 'free'].values[0],
            'usd_balance' : balances.loc[balances['asset'] == 'USDT', 'free'].values[0]
        }
...
if __name__ == "__main__":
    ...
    client.SYNC_THREADING = False  # Set SYNC_THREADING to False to use synchronous requests
    ...
    def stop_trading():
        ...
        if stop_trading_flag is False:
            ...
            return jsonify({'message': 'Trading has been stopped'})
        else:
            return jsonify({'message': 'Trading is already stopped'})
